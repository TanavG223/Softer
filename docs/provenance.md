...
| Native release compilation | PASS with Apple Command Line Tools and `swift build -c release --product Softer` |
...
| Packaged app launch and risk-based walkthrough | PASS: the signed Softer bundle loaded the encrypted profile created under the former name, started and stopped a standard activity, ran and paused the breathing pacer, switched pace and shape-only wording, placed/hinted/undid Harbor Tiles, rendered both game choices and all support routes, and visibly reported version 0.4.0; see `release_acceptance_2026-09-04.md` |
| v0.4.0 rename archive | PASS: `Softer-0.4.0-macOS-universal.zip` extracted cleanly; strict signature verification passed; both arm64 and x86_64 slices report macOS 14.0 minimum; SHA-256 is recorded in `release_v0.4.0.md` |
| Submission screenshots | PASS: current Retina captures for Calm, Play, Harbor Tiles, and Gentle breathing hide the profile sidebar and expose no alias in `output/submission/` |
...
Evidence-source and competitor-repository links are recorded in `evidence_manifest.json`, `mental_wellbeing_evidence_contract.md`, `mental_wellbeing_game_research.md`, and `competitive_ux_research.md`. Those sources inform design and claims; they did not test Softer. The public site's accessibility pass preserves native link semantics for support actions and corrected the computed text/background pairs on the game, support, and research surfaces in both color schemes. A Codex-browser audit found no text contrast failures, horizontal overflow at 375 px, undersized visible controls, or browser console errors after the patch.
