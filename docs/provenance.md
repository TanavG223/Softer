...
| v0.3.1 patch archive | PASS: `PaceBack-0.3.1-macOS-universal.zip` extracted cleanly; strict signature verification passed; both arm64 and x86_64 slices report macOS 14.0 minimum; the packaged About screen visibly reported 0.3.1; SHA-256 is recorded in `release_v0.3.1.md` |
...
Evidence-source and competitor-repository links are recorded in `evidence_manifest.json`, `mental_wellbeing_evidence_contract.md`, `mental_wellbeing_game_research.md`, and `competitive_ux_research.md`. Those sources inform design and claims; they did not test PaceBack. The public site's v0.3.1 accessibility pass preserves native link semantics for support actions and corrected the computed text/background pairs on the game, support, and research surfaces in both color schemes. A Codex-browser audit found no text contrast failures, horizontal overflow at 375 px, undersized visible controls, or browser console errors after the patch.
